...
ax6 = nexttile; ax7 = nexttile; ax8 = nexttile;
figure(2)
...
cone = ConePhotoReceptor(cone0, buffer_size, dt, method, gap_junc, g_vals);
rod = RodPhotoReceptor_RK(rod0, buffer_size, dt, method, gap_junc, g_vals);
rod2 = RodPhotoReceptor_RK(rod0, buffer_size, dt, method, gap_junc, g_vals);
rod_bip = Bipolar_complete(rbip0, buffer_size, dt, method);
cone_bip = Bipolar_complete(cbip0, buffer_size, dt, method, gap_junc, g_vals2);
amacrine = Amacrine(a0, buffer_size, dt, method, gap_junc, g_vals2);
...
while abs(curr_t - t_end) > eps
    ...
    [y_rod, curr_t, c_rod] = rod.solve(in_rod_interp, [v_cone+offset, v_rod2]);
    [y2_rod, ~, ~] = rod2.solve(in_rod_interp, [v_cone+offset, v_rod]);
    [y_cone, ~, ~] = cone.solve(in_cone_interp, [v_rod-offset, v_rod2-offset]);
...
plot(ax8, t_vec(1:t_vec_end), v_rod2(1:t_vec_end)); title(ax8, 'Rod2 Membrane Potential');
xlabel(ax8, 't [s]'); ylabel(ax8, 'V_{rod} [mV]')
xlim(ax8, [t_vec(1), t_vec(t_vec_end)])
grid(ax8, 'on'); grid(ax8, 'minor')
hold(ax8, 'on')
