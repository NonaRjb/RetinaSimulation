classdef Bipolar_complete < handle
    ...
    properties
		...
		Cablmax = 400;  % [uM] Total low-affinity buffer concentration
        ...
        Esyn = 0;       % [mV] Synapse's reversal potential
        tau = 10;       % [ms] Time constant
        Vslope = 10;    % [mV] Voltage sensitivity of the synapse
        Vth = -50;      % [mV]
        % variables
        Y
    end
    ...
    methods
        function obj = Bipolar_complete(Y0, buffer_size, dt)
        ...
        function [y, curr_t, c]  = solve(obj,Vpre)
			...
			hCa = exp(-(obj.Y(1, k)-50)/11)/(exp(-(obj.Y(1, k)-50)/11)+1);
			...
			Iex = obj.Jex*(obj.Y(13, k)-obj.Camin)/(obj.Y(13, k)...
                -obj.Camin+2.3)*exp(-(obj.Y(1, k)+14)/70);
...
D(4) = am_A*(1-Y(4))-bm_A*Y(4);
D(5) = ah_A*(1-Y(5))-bh_A*Y(5);
D(6) = -4*ah*Y(6)+bh*Y(7);
...
D(11) = am_Ca*(1-Y(11))-bm_Ca*Y(11);
D(12) = am_KCa*(1-Y(12))-bm_KCa*Y(12);
D(13) = -iCa/(2*F*Vs)-DCa*Ssd/(Vs*dsd)*(Y(13)-Y(14))-(Iex+Iex2)/(2*F*Vs)+...
    bbl*Y(15)-abl*Y(13)*(Cablmax-Y(15))+bbh*Y(16)-abh*Y(13)*(Cabhmax-Y(16));
D(14) = DCa*Ssd/(Vd*dsd)*(Y(13)-Y(14))+bbl*Y(17)-abl*Y(14)*(Cablmax-Y(17))+...
    bbh*Y(18)-abh*Y(14)*(Cabhmax-Y(18));
D(15) = abl*Y(13)*(Cablmax-Y(15))+bbl*Y(15);
D(16) = abh*Y(13)*(Cabhmax-Y(16))+bbh*Y(16);
D(17) = abl*Y(14)*(Cablmax-Y(17))+bbl*Y(17);
D(18) = abh*Y(14)*(Cablmax-Y(18))+bbh*Y(18);
